...
use gtk::{
    glib::{self, clone},
    prelude::*,
     ApplicationWindow, Button, Grid, Label,
...
pub fn table_ui(application: &adw::Application) -> ApplicationWindow {
                ...
                delete_dialog.connect_response(move |delete_dialog, resp| {
                    ...
                    if let Some((model,selected_row)) = selection.selected() {
                        if resp == ResponseType::Ok {
                            ...
                            remove_account(website.parse().unwrap()).expect("Remove account failed");
                        }
                        delete_dialog.close();
                    }});
                delete_dialog.show()
    ...
    let reset_pass_button = Button::builder()
        ...
        reset_pass_button.connect_clicked(clone! (@weak table_window, @weak application => move |_| {
            ...
            let password_entry = gtk::Entry::new();
            password_entry.set_placeholder_text(Some("New Password"));
            ...
            let password_entry_compare = gtk::Entry::new();
            password_entry_compare.set_placeholder_text(Some("Re-enter Password"));
            password_entry_compare.connect_activate(clone!(@weak reset_pass_dialog => move |_| {
            reset_pass_dialog.response(ResponseType::Ok);
            }));
            content_area.append(&password_entry_compare);

            reset_pass_dialog.connect_response(clone!(@weak password_entry, @weak password_entry_compare, @weak table_window, @weak _list_store => move |reset_pass_dialog, resp| {
                if (password_entry.text() == password_entry_compare.text()) && resp == ResponseType::Ok {
                    ...
                    for account in manager_account_list_json_struct.account_list.iter_mut() {
                        if account.website !="master password" && account.username !="master password" {
                            ...
                            let old_tag = account.tag.clone();
                            let old_master_pass = get_master_password();

                            let decrypted_pass_old = chacha20poly1305_decrypt_string(
                                old_master_pass,
                                old_pass,
                                old_tag);

                            //encrypt each account pass with new password and re-insert
                            let (encrypted_pass_new, encrypted_tag_new) = chacha20poly1305_encrypt_string(new_password.clone(), String::from_utf8(decrypted_pass_old).expect("Found invalid UTF-8"));
                            account.password = encrypted_pass_new;
                            account.tag = encrypted_tag_new;
                        }
                    }
                    //save all changes to json
                    std::fs::write("password_manager_json.json", serde_json::to_string(&manager_account_list_json_struct.clone()).unwrap()).expect("JSON failed to load");
                }
